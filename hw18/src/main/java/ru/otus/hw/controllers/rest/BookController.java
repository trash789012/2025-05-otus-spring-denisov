...
public class BookController {
    ...
    @CircuitBreaker(name = "findAll", fallbackMethod = "fallbackGetAllBooks")
    ...
    public List<BookDto> fallbackGetAllBooks(Throwable throwable) {
        //просто чтобы проверить, что это вызывается, логику не закладываю
        return List.of();
    }

    @GetMapping("/api/v1/book/{id}")
    @CircuitBreaker(name = "getBookById")
    @RateLimiter(name = "bookService")
    ...
    @CircuitBreaker(name = "createBook")
    @RateLimiter(name = "bookService")
